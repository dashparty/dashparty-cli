...
def extract_old_config():
    ...
    old_appdir = appdirs.user_config_dir(appauthor='Dashparty', appname='dashpartyd', roaming=True)
    old_configfile = os.path.join(old_appdir, 'dashpartyd.conf')
...
def extract_bitcoincore_config():
    ...
    if platform.system() == 'Darwin':
        btc_conf_file = os.path.expanduser('~/Library/Application Support/Dash/')
    elif platform.system() == 'Windows':
        btc_conf_file = os.path.join(os.environ['APPDATA'], 'Dash')
    else:
        btc_conf_file = os.path.expanduser('~/.dash')
    btc_conf_file = os.path.join(btc_conf_file, 'dash.conf')
...
def server_to_client_config(server_config):
    ...
    config_keys = {
        ...
        'rpc-host': 'dashparty-rpc-connect',
        'rpc-port': 'dashparty-rpc-port',
        'rpc-user': 'dashparty-rpc-user',
        'rpc-password': 'dashparty-rpc-password'
    }
...
def generate_config_files():
    from dashpartycli.server import CONFIG_ARGS as SERVER_CONFIG_ARGS
    from dashpartycli.client import CONFIG_ARGS as CLIENT_CONFIG_ARGS
    from dashpartylib.lib import config, util
...
def before_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        shutil.copy('dashpartycli/__init__.py', 'dashparty-{}.py'.format(exe_name))
        with open('dashparty-{}.py'.format(exe_name), 'a') as fp:
...
def after_py2exe_build(win_dist_dir):
    ...
    for exe_name in ['client', 'server']:
        os.remove('dashparty-{}.py'.format(exe_name))
...
def bootstrap(overwrite=True, ask_confirmation=False):
    if ask_confirmation:
        question = 'Would you like to bootstrap your local Dashparty database from `https://s3.amazonaws.com/dashparty-bootstrap/`? (y/N): '
