...
from dashpartylib.lib import log
logger = logging.getLogger(__name__)

from dashpartylib.lib import config, script
from dashpartylib.lib.util import make_id, BET_TYPE_NAME
from dashpartylib.lib.log import isodt
from dashpartylib.lib.exceptions import TransactionError
from dashpartycli.util import add_config_arguments
from dashpartycli.setup import generate_config_files
from dashpartycli import APP_VERSION, util, messages, wallet, console, clientapi

APP_NAME = 'dashparty-client'
...
CONFIG_ARGS = [
    ...
    [('--dashparty-rpc-connect',), {'default': 'localhost', 'help': 'the hostname or IP of the Dashparty JSON-RPC server'}],
    [('--dashparty-rpc-port',), {'type': int, 'help': 'the port of the Dashparty JSON-RPC server'}],
    [('--dashparty-rpc-user',), {'default': 'rpc', 'help': 'the username for the Dashparty JSON-RPC server'}],
    [('--dashparty-rpc-password',), {'help': 'the password for the Dashparty JSON-RPC server'}],
    [('--dashparty-rpc-ssl',), {'default': False, 'action': 'store_true', 'help': 'use SSL to connect to the Dashparty server (default: false)'}],
    [('--dashparty-rpc-ssl-verify',), {'default': False, 'action': 'store_true', 'help': 'verify SSL certificate of the Dashparty server; disallow use of self‐signed certificates (default: false)'}],
...
def main():
    if os.name == 'nt':
        from dashpartylib.lib import util_windows
    ...
    parser = argparse.ArgumentParser(prog=APP_NAME, description='Dashparty CLI for dashparty-server', add_help=False)
    parser.add_argument('-h', '--help', dest='help', action='store_true', help='show this help message and exit')
    parser.add_argument('-V', '--version', action='version', version="{} v{}; {} v{}".format(APP_NAME, APP_VERSION, 'dashparty-lib', config.VERSION_STRING))
    ...
    parser_destroy = subparsers.add_parser('destroy', help='destroy a quantity of a Dashparty asset')
    ...
    parser_getrows = subparsers.add_parser('getrows', help='get rows from a Dashparty table')
    ...
    clientapi.initialize(testnet=args.testnet, testcoin=args.testcoin, regtest=args.regtest, customnet=args.customnet,
                        counterparty_rpc_connect=args.dashparty_rpc_connect, counterparty_rpc_port=args.dashparty_rpc_port,
                        counterparty_rpc_user=args.dashparty_rpc_user, counterparty_rpc_password=args.dashparty_rpc_password,
                        counterparty_rpc_ssl=args.dashparty_rpc_ssl, counterparty_rpc_ssl_verify=args.dashparty_rpc_ssl_verify,
