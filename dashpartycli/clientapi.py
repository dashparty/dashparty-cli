...
from dashpartycli import util
from dashpartycli import wallet
from dashpartycli import messages
from dashpartycli.messages import get_pubkeys
...
def initialize(testnet=False, testcoin=False, regtest=True, customnet="",
    ...
    try:
        ...
        raise Exception("Please specific a valid port number dashparty-rpc-port configuration parameter")
    ...
    # BTC Wallet name
    config.WALLET_NAME = wallet_name or 'dashcore'
    ...
    if config.TESTCOIN:
        ...
        config.PREFIX = b'DASHPRTY'             # 8 bytes
...
def call(method, args, pubkey_resolver=None):
    """
        ...
        Should be used by applications like `dashparty-gui`

        :Example:

        import dashpartycli.clientapi
