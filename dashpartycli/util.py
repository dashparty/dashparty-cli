...
from dashpartylib.lib import config, check
from dashpartylib.lib.util import value_input, value_output
...
def bootstrap(testnet=False, overwrite=True, ask_confirmation=False, quiet=False):
    ...
    if testnet:
        if check.CONSENSUS_HASH_VERSION_TESTNET < 7:
            BOOTSTRAP_URL = 'https://dashparty.io/bootstrap/dashparty-db-testnet.latest.tar.gz'
        else:
            BOOTSTRAP_URL = 'https://dashparty.io/bootstrap/dashparty-db-testnet-{}.latest.tar.gz'.format(check.CONSENSUS_HASH_VERSION_TESTNET)
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'dashpartyd-testnet-db.latest.tar.gz')
        DATABASE_PATH = os.path.join(data_dir, '{}.testnet.db'.format(config.APP_NAME))
    else:
        if check.CONSENSUS_HASH_VERSION_MAINNET < 3:
            BOOTSTRAP_URL = 'https://dashparty.io/bootstrap/dashparty-db.latest.tar.gz'
        else:
            BOOTSTRAP_URL = 'https://dashparty.io/bootstrap/dashparty-db-{}.latest.tar.gz'.format(check.CONSENSUS_HASH_VERSION_MAINNET)
        TARBALL_PATH = os.path.join(tempfile.gettempdir(), 'dashpartyd-db.latest.tar.gz')
